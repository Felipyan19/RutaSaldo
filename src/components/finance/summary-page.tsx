...
import { accountBalance, formatCOP, totals } from "@/lib/finance";
...
export function SummaryPage() {
  ...
  return <>
    ...
    <section className="grid gap-4 md:grid-cols-3"><Metric label="Saldo total" value={formatCOP(summary.balance)} note={`${state.accounts.length} cuentas activas`} tone="dark" /><Metric label="Ingresos" value={formatCOP(summary.income)} note="Total registrado" tone="green" /><Metric label="Gastos" value={formatCOP(summary.expenses)} note={`${variation}% disponible del ingreso`} tone="light" /></section>
