...
export function TransactionList({ state, limit }: { state: FinanceState; limit?: number }) {
  ...
  return <div className="divide-y divide-[#edf0eb]">{items.map((transaction) => {
    ...
    return <div key={transaction.id} className="flex items-center gap-3 py-4 first:pt-0 last:pb-0">
      ...
      <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{transaction.description}</p><p className="mt-0.5 truncate text-xs text-[#5e6d63]">{transfer ? "Transferencia" : category?.name ?? "Sin categoría"} · {account?.institution ?? "Cuenta eliminada"}</p></div>
      <div className="text-right"><p className={`text-sm font-semibold ${income || incomingTransfer ? "text-[#3f795e]" : ""}`}>{income || incomingTransfer ? "+" : "−"}{formatCOP(transaction.amount)}</p><p className="mt-0.5 text-[10px] text-[#5e6d63]">{new Date(`${transaction.date}T12:00:00`).toLocaleDateString("es-CO", { day: "numeric", month: "short" })}</p></div>
    </div>;
  })}</div>;
}
