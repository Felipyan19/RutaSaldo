...
export function MetricCard({ icon: Icon, label, value, note, tone = "neutral", featured = false }: { icon: LucideIcon; label: string; value: string; note?: string; tone?: "neutral" | "positive" | "negative" | "warning"; featured?: boolean }) {
  ...
  return <article className={`rounded-2xl border border-[#e0e4dd] bg-white ${featured ? "p-5 sm:p-6" : "p-4"}`}>
    <span className={`grid h-9 w-9 place-items-center rounded-xl ${tones[tone]}`}><Icon size={17} aria-hidden="true" /></span>
    <p className="mt-4 text-xs text-[#5e6d63]">{label}</p>
    <p className={`mt-1 break-words font-semibold tracking-tight ${featured ? "text-2xl" : "text-xl"}`}>{value}</p>
    {note && <p className="mt-2 text-xs leading-5 text-[#6b786f]">{note}</p>}
  </article>;
}
...
export function Panel({ title, description, children, className = "" }: { title?: string; description?: string; children: React.ReactNode; className?: string }) {
  return <section className={`rounded-3xl border border-[#e0e4dd] bg-white p-5 md:p-7 ${className}`}>
    {(title || description) && <div className="mb-5"><h2 className="text-lg font-semibold">{title}</h2>{description && <p className="mt-1 text-xs leading-5 text-[#5e6d63]">{description}</p>}</div>}
    {children}
  </section>;
}

export function EmptyState({ children }: { children: React.ReactNode }) {
  return <div className="rounded-2xl border border-dashed border-[#cbd5cc] p-6 text-center text-sm leading-6 text-[#5e6d63]">{children}</div>;
}
...
export function AnalysisBar({ label, value, percentage, color }: { label: string; value: string; percentage: number; color: string }) {
  ...
  return <div role="img" aria-label={`${label}: ${value}, ${Math.round(safe)}% respecto al valor mayor`}>
    <div className="mb-1.5 flex items-center justify-between gap-3 text-xs"><span className="min-w-0 truncate font-medium">{label}</span><span className="shrink-0 font-semibold">{value}</span></div>
